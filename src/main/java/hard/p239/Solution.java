...
class Solution {
    ...
    private PriorityQueue<Integer> leftQueue = new PriorityQueue<Integer>(comparator);
    private PriorityQueue<Integer> fullQueue = new PriorityQueue<Integer>(comparator);
    ...
    public int[] maxSlidingWindow(int[] nums, int k) {
        ...
        // init
        for (int i = 0; i < k; i++) {
            fullQueue.offer(nums[i]);
        }
        result[0] = fullQueue.peek();

        // start moving
        for (int i = 0; i < nums.length - k; i++) {
            leftQueue.offer(nums[i]);
            fullQueue.offer(nums[i + k]);

            while (fullQueue.peek().equals(leftQueue.peek())) {
                fullQueue.poll();
                leftQueue.poll();
            }
            // max is at right size(moving window)
            result[i + 1] = fullQueue.peek();
        }
        return result;
